...
class MatchLoss(object):
  ...
  def run(self, global_step, data, logits, ys, e_hat, y_hat):
    ...
    for i in range(len(logits)):
        gt_geod_d = ys[i]
        is_pos = (gt_geod_d < self.obj_geod_th).type(gt_geod_d.type())  # [32*2000] 内点=1，外点=0
        is_neg = (gt_geod_d >= self.obj_geod_th).type(gt_geod_d.type())  # [32*2000] 内点=0，外点=1
        with torch.no_grad():
    ...
    is_pos_all = (gt_geod_d_all < self.obj_geod_th).type(gt_geod_d_all.type())  # [32*2000] 内点=1，外点=0
