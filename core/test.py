...
def test_cost(data_xs, data_ys, model, iter_num):
    with torch.no_grad():
        torch.cuda.synchronize()
        start=time.time()  #记录开始时间
        for _ in range(int(iter_num)):
            _ = model(data_xs, data_ys)
        torch.cuda.synchronize()
        end=time.time()  #记录结束时间
        training_time=end - start
    print('Average time per run(ms):',training_time/iter_num*1e3)
...
def test_process(mode, model, cur_global_step, data_loader, config):
    ...
    with torch.no_grad(): 
        for test_data in loader_iter:
            ...
            if test_xs.shape[2]<1000:
                continue;

            logits, ys_ds, e_hat, y_hat = model(test_xs, test_ys)

            # test_cost(test_xs, test_ys, model, 100)

            #y_hat, e_hat = res_logits[-1], res_e_hat[-1]
            loss, geo_loss, cla_loss, l2_loss, prec, rec = match_loss.run(cur_global_step, test_data, logits, ys_ds, e_hat, y_hat)
